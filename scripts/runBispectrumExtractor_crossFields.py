...
with open(infiles, 'r') as file:
    filenames = [line.strip().split() for line in file]


# INITIALIZATION EXTRACTOR

Xtract=BiG.bispectrumExtractor(L, Nmesh, kbinedges, args.verbose, singleField=False)
...
for f in filenames:
    ...
    field_real1=loader.load(f[0])
    field_real2=loader.load(f[1])
    field_real3=loader.load(f[2])


    bispec=Xtract.calculateBispectrum(field_real1, field_real2=field_real2, field_real3=field_real3, mode=mode)
